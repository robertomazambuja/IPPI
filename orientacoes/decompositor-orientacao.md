...
Um arquivo `instrucoes.json` com **um objeto por capítulo**. Você produz apenas a **arquitetura cognitiva**; o `pipeline.py` serializa o `instrucoes.csv` final preenchendo os campos de contexto a partir do briefing e da matriz.

| Campo | Você gera? | Origem |
|---|---|---|
| `capitulo` | Sim (copia do briefing) | briefing — chave de casamento, idêntica |
| `secoes[].micro_hab` | **Sim** (sua tarefa central) | você gera (4 a 6 por capítulo) |
| `secoes[].operacao` | **Sim** | você define seguindo `sequencia_pedagogica` |
| `disciplina`, `unidade`, `pergunta_unidade` | Não | pipeline, do briefing |
| `habilidade` (código + enunciado) | Não | pipeline, da matriz (fonte de verdade) |
| `autores` | Não | pipeline, de `autores_por_capitulo[capitulo]` |
| `conteudos_nucleares` | Não | pipeline, de `conteudos_por_capitulo[capitulo]` |

Você não escreve CSV, não recopia o enunciado e não mapeia autores/conteúdos — isso é feito de forma determinística pelo pipeline, eliminando erros de formatação e divergência da matriz.
...
## Autores e conteúdos por capítulo

O professor já fez a distribuição por capítulo no briefing, e o **pipeline** copia `autores_por_capitulo[capitulo]` e `conteudos_por_capitulo[capitulo]` para o CSV automaticamente, casando pelo nome do capítulo. Por isso é essencial que o `capitulo` no seu JSON seja **idêntico** ao do briefing.

Você não redistribui, filtra nem interpreta esses dados — nem precisa copiá-los. O Agente 1 decide como usar cada autor e cada conteúdo dentro das seções do capítulo.
